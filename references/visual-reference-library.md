...
| Visual grammar | Read `visual-grammar.md`; store a completed `visual_grammar` card in metadata, including connector/object/repetition treatment when present and exact `not_present` for absent families |
...
| Output | Sidecar metadata (`visual_grammar`, `code_path`, `reproduction_preview_path`) + copied image + reconstruction preview + rebuilt `assets/registry.jsonl`; never hand-edit the registry |
...
Before choosing a normalized type, read `figure-family-coverage.md` and record
the evidence family/subtype. Use `python scripts/figure_family_coverage.py` to
see which families have reviewed references and which require `build_new`.
...
The intake lifecycle is persisted separately as `lifecycle_state`: `raw → analyzed → reviewed → benchmarked → production` (or `rejected`). `reviewed` is visible to diagnostic queries but is not eligible for strict production recommendation until `benchmark_reference()` records a passing retrieval/generation canary. `promote_reference()` additionally records champion-floor evidence and sets `production_ready=true`. New sidecars therefore carry a `quarantine` object with the current state and transition evidence.

Legacy `code.py` files are audit-only assets. Intake never imports or executes them. An explicit private reproduction audit must call `scripts/reference_code_sandbox.py`, which performs import/call checks and runs inside a temporary directory with network/process imports rejected and a timeout.
...
```bash
python scripts/prepare_visual_optimization.py \
  --before current-figure.png --output-dir optimization-packet \
  --figure-type Heatmap \
...
The packet owns the recommendation call. The underlying library command remains
available for library maintenance and diagnostics, but is not a second production route:
