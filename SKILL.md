...
1. Render and open the current figure; record structural, hierarchy, whitespace, density, legend, and legibility failures. For a mechanism, architecture, image-rich plate, or non-trivial multi-panel figure, read `references/art-direction.md` and select one direction; use `analytic_minimalism` for a simple plot-led result.
2. **Do not edit plotting source yet.** Materialize one fresh task-local packet with `python <skill-root>/scripts/prepare_visual_optimization.py --before <current.png> --figure-type <normalized-type> --output-dir <new-packet-dir> [--preferred-tags <tags>] [--art-direction <direction>]`. The packet runs `scripts/reference_library.py recommend` and returns at most 3 candidates. Read its `RUNBOOK.md`; it contains the exact, task-specific candidate image paths, a saved recommendation report, and the only contract to complete. `<skill-root>` is the directory containing this `SKILL.md`.
3. Open every shortlisted candidate's actual pixels and record one visible observation per candidate in the packet contract. Then choose one reference. For a heterogeneous multi-panel figure, create a packet separately for the assembly skeleton and each distinct panel family; do not apply one shortlist to every panel. Fewer than three candidates is valid; never fill the shortlist with another figure type. If none is compatible, record `build_new` and why.
4. Complete the Reference Reconstruction Contract plus the Visual Optimization Contract in `references/reference-driven-reconstruction.md` before editing plotting code. The optimization record must state a fresh `palette_decision`, one `art_direction` with an evidence-based reason, and a `composition_decision` that explicitly rejects the old figure skeleton (hero evidence, support evidence, and what old equal-weight grid/legend structure is removed); old hex values may be retained only with an explicit semantic and contrast justification.
5. Make at least one evidenced structural or encoding change. Palette/font/alpha/line-width/marker-size/spacing-only changes are cosmetic and cannot satisfy an optimization request.
6. Render at final physical size, create an equal-cell `Before | Reference | After` comparison, inspect it, run `scripts/rendered_contrast.py` for every cell/tile annotation, and run `scripts/check_visual_optimization.py`. A FIX verdict blocks delivery. Passing source checks while retaining the old equal-weight subplot skeleton is a FIX, not a successful optimization.
...
| Art direction for a mechanism, architecture, image plate, or non-trivial multi-panel figure | `references/art-direction.md`; select exactly one compatible direction |
...
| Visual optimization | Run `scripts/prepare_visual_optimization.py` first; then `references/art-direction.md`, `references/reference-driven-reconstruction.md`, `references/visual-reference-library.md`; record `art_direction` and a `palette_decision`, run `scripts/rendered_contrast.py`, then `scripts/check_visual_optimization.py` |
