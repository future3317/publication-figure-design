...
def visual_grammar():
    return {
        "canvas_composition": {
            "aspect_and_panel_layout": "wide horizontal explanatory sequence",
            "visual_hierarchy": "a geometric hero precedes smaller supporting stages",
            "alignment_and_spacing": "stages share a baseline with deliberate open gaps",
        },
        "connectors": {
            "geometry": "sparse straight and gently curved arrow connectors",
            "direction_and_arrowhead": "small terminal arrowheads point left to right",
            "stroke": "thin muted purple/red strokes, lighter than the hero object outline",
            "anchors_and_routing": "arrows leave object edges and route through open whitespace",
            "layering": "connectors sit behind labels and in front of the pale background",
        },
        "objects_material": {
            "shape_and_projection": "overlapping sphere-like geometric objects with simple depth cues",
            "fill_and_material": "flat colored fills with a restrained highlight",
            "outline_and_edges": "dark thin outline separates the objects from the canvas",
            "depth_cues": "occlusion and one highlight communicate depth; no photorealistic shadow",
            "placement_and_scale": "largest object is the visual hero; later objects are smaller stages",
        },
        "repetition_structures": {
            "topology": "horizontal sequence of three explanatory stages",
            "count_spacing_rhythm": "even stage rhythm with larger whitespace around the hero",
            "grouping_and_alignment": "each stage pairs its object and local label",
            "variation_and_emphasis": "only the lead stage uses the strongest color and largest scale",
        },
        "palette_roles": {
            "background": "white with no decorative field",
            "roles_and_proportions": "mostly neutral outlines; purple/red connectors are sparse accents",
            "contrast_and_emphasis": "the hero fill is more saturated than support stages",
        },
        "annotations_typography": {
            "text_hierarchy": "short local labels in a compact sans-serif hierarchy",
            "callouts_and_leaders": "labels follow their stage; arrows explain transitions",
            "placement_and_clearance": "labels occupy open whitespace and never cross object edges",
        },
        "legend_key": "not_present",
        "chart_marks_axes": "not_present",
        "must_match": ["sparse left-to-right connectors", "sphere-like hero with restrained depth cues"],
    }
...
def contract(**overrides):
    ...
    value = {
        ...
        "selected_reference_visual_grammar": visual_grammar(),
...
class VisualOptimizationTests(unittest.TestCase):
    def setUp(self):
        ...
        # The valid fixture intentionally aligns the final raster with the
        # selected reference; the new visual-quality gate must not accept a
        # merely different after image.
        Image.new("RGB", (50, 40), "#eeeeee").save(self.after)
    ...
    def test_low_reference_alignment_blocks_even_complete_contract(self):
        import scripts.check_visual_optimization as checker

        original = checker.compare_output_to_reference
        checker.compare_output_to_reference = lambda *_: {
            "verdict": "fail",
            "metrics": {"overall_style_similarity": 0.21},
        }
        try:
            report = validate_visual_optimization(
                self.valid_contract(), self.before, self.after, self.reference, self.comparison
            ...
            checker.compare_output_to_reference = original
        self.assertFalse(report["ready"])
        self.assertFalse(report["checks"]["reference_alignment_gate"])
        self.assertIn("reference-alignment", " ".join(report["errors"]))

    def test_selected_reference_visual_grammar_is_required(self):
        value = self.valid_contract()
        value.pop("selected_reference_visual_grammar")
        report = validate_visual_optimization(
    ...
    def test_passing_rendered_contrast_report_allows_text_on_fill(self):
        ...
        # Keep the selected reference visually aligned so this test isolates
        # the rendered-contrast gate rather than the fidelity gate.
