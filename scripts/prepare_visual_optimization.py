...
def _visual_grammar_skeleton() -> dict[str, Any]:
    """Return the observation-card slots completed from opened reference pixels."""
    return {
        "canvas_composition": {
            "aspect_and_panel_layout": None,
            "visual_hierarchy": None,
            "alignment_and_spacing": None,
        },
        ...
        "palette_roles": {
            "background": None,
            "roles_and_proportions": None,
            "contrast_and_emphasis": None,
        },
        "annotations_typography": {
            "text_hierarchy": None,
            "callouts_and_leaders": None,
            "placement_and_clearance": None,
        },
...
def _runbook(
    ...
    return "\n".join((
        ...
        *candidate_lines,
        f"3. Read `{skill_root / 'references' / 'visual-grammar.md'}` and fill the selected-reference visual grammar in `{contract}`. Record every applicable family at implementation detail (for example connector path/arrowhead/stroke/anchors, object material/depth, or repetition rhythm); set only absent families to `not_present`.",
        f"4. Read `{skill_root / 'references' / 'art-direction.md'}` and select one art direction in `{contract}`; do not blend directions without declared panel roles.",
        f"5. Fill `{contract}` with the selected reference, old-skeleton rejection, fresh palette decision, and intended structural changes.",
        "6. Only then edit the plotting source; make at least one structural or encoding change, not cosmetic restyling.",
        "7. Render the after figure at final physical size, build an equal-cell Before | Reference | After comparison, then run:",
...
def prepare_packet(
    *,
    ...
    contract = {
        ...
        "selected_reference_visual_grammar": _visual_grammar_skeleton(),
