...
class SelfCheckTests(unittest.TestCase):
    ...
    def test_self_check_rejects_numbered_production_successors(self):
        from check_skill_contract import validate_skill
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "SKILL.md").write_text(
                ...
                "exact_reuse structural_adaptation style_only build_new\n"
                "panel topology\nmark geometry\nlayer topology\ndata encoding\n"
                "annotation/legend model\nfinal assembler\n",
                encoding="utf-8",
            )
            ...
            (root / "scripts" / "plot_v2.py").write_text("", encoding="utf-8")
            report = validate_skill(root)
        self.assertFalse(report["ok"])
        self.assertTrue(any("iteration-suffixed filename" in error for error in report["errors"]))
